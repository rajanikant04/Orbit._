...
export const createPost = async (req, res) => {
    try {
        ...
        await newPost.save();
        console.log("Post created successfully in post controller");
...
export const deletePost = async (req, res) => {
    try {
        ...
        if(!post) {
            console.log("Post not found in post.controller");
        ...
        console.log("post deleted in post.controller");
...
export const commentOnPost = async (req, res) => {
    try {
        ...
        await post.save();
        console.log("comment done successfully in controller");
...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        if(userLikedPost) {
            ...
            // here we have created an array of likes 
            const updatedLikes = post.likes.filter((id) => id.toString() !== userId.toString());
            console.log("post unliked in post.controller");
...
export const getAllPosts = async(req, res) => {
    try {
        const posts = await Post.find().sort({createdAt: -1}).populate({   // populate is used to show the datas from other table to this directly
            path: "user",
            select:"-password"
        })
        .populate({
